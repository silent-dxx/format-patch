...
class PatchSet(object):
  ...
  def format_symbol(self, text, s):
    text = re.sub(bytes('(?<!(\{}|\s))\{}(?!(\{}|\s))'.format(s, s, s), encoding='utf8'), bytes(' {} '.format(s), encoding='utf8'), text)
    text = re.sub(bytes('(?<!\{})\{}(?!(\{}|\s))'.format(s, s, s), encoding='utf8'), bytes('{} '.format(s), encoding='utf8'), text)
    text = re.sub(bytes('(?<!(\{}|\s))\{}(?!(\{}))'.format(s, s, s), encoding='utf8'), bytes(' {}'.format(s), encoding='utf8'), text)
    return text
  ...
  def format_line(self, text):
    ...
    text = re.sub(b'if\(', b'if (', text)
    text = re.sub(b'while\(', b'while (', text)
    text = re.sub(b'do\(', b'do (', text)
    text = re.sub(b'\)\{', b') {', text)
    text = re.sub(b'\}else', b'} else', text)
    text = re.sub(b'else\{', b'else {', text)
    text = re.sub(b',(?!(\n|\s|\'))', b', ', text)

    '''
    if text[0] == 43:
      text = self.format_symbol(text[1:], '+')
      text = b'+' + text
    else:
      text = self.format_symbol(text, '+')
  ...
  def dump(self):
    for p in self.items:
      for headline in p.header:
        print(headline.decode(encoding='utf-8').rstrip('\n'))
      print('--- ' + p.source.decode(encoding='utf-8'))
      print('+++ ' + p.target.decode(encoding='utf-8'))
      for h in p.hunks:
        desc = ""
        if len(h.desc):
          desc = " " + h.desc.decode(encoding='utf-8')
        print('@@ -%s,%s +%s,%s @@%s' % (h.startsrc, h.linessrc, h.starttgt, h.linestgt, desc))
        for line in h.text:
